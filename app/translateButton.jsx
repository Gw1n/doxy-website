...
export default function TranslateButton({ lang }) {
    ...
    return (
        <div>
            <Menu>
                <MenuButton className="border-2 border-purple-700 rounded-xl bg-white/20 px-3 py-2 text-md cursor-pointer flex gap-2 data-focus:outline-none focus:not-data-focus:outline-none w-40 text-white items-center text-xl z-20">
                    {({ open }) => (
                        <>

                            <currentLanguage.FlagComponent width={22} height={22} />
                            <span>{currentLanguage.name}</span>
                            <ChevronDownIcon className={`h-7 w-7 transition-transform duration-500 ease-in-out ml-auto mt-1 ${open ? 'rotate-180' : ''}`} />
                        </>
                    )}
                </MenuButton>
                ...
                >
                    <MenuItems transition anchor="bottom start" className="text-sm text-white origin-top p-2 rounded-xl bg-purple-950/20 backdrop-blur-md data-focus:outline-none focus:not-data-focus:outline-none flex mt-1 flex-col gap-y-2 w-40">
                        {languages.map((language) => (
                            <MenuItem key={language.code}>
                                <Link
                                    href={redirectedPathName(language.code)}
                                    className="border-2 border-purple-700 rounded-md bg-white/10 px-3 py-1 text-md cursor-pointer justify-start items-center flex gap-4 w-full hover:bg-white/20 transition"
                                >
                                    <language.FlagComponent width={24} height={24} />
